...
{
    public class MotorcycleThirdPartyIntegrationTests
    {
        ...
        [Fact]
        public async Task MotorcycleThirdParty_RunAsync_ShouldReturnTrue()
        {
            var client = CreateClient();
            await AuthenticateAsync(client);
            var result = await new MotorcycleThirdPartyInsuranceSamples(client).RunAsync();
