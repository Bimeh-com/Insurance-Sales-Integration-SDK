...
{
    public class PilgrimageIntegrationTests
    {
        ...
        [Fact]
        public async Task Pilgrimage_RunAsync_ShouldReturnTrue()
        {
            var client = CreateClient();
            await AuthenticateAsync(client);
            var result = await new PilgrimageInsuranceSamples(client).RunAsync();
