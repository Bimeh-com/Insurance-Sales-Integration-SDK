...
{
    public class TravelPlusIntegrationTests
    {
        ...
        [Fact]
        public async Task TravelPlus_RunAsync_ShouldReturnTrue()
        {
            var client = CreateClient();
            await AuthenticateAsync(client);
            var result = await new TravelPlusInsuranceSamples(client).RunAsync();
